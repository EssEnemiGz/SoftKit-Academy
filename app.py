...
from flask import Flask, render_template, abort, session, redirect, request, send_from_directory
...
app.permanent_session_lifetime = timedelta(
    weeks=52
)  # Sesion con duracion de 52 semanas o 1 año
...
auth_key = db.auth.sign_in_with_password(
    {"email": admin_email, "password": admin_passw}
)
if auth_key.session:
...
def insert_route():
    if not len(session):
        ...
        return render_template("es/insert.html")
...
def students_panel_default():
    ...
    r = requests.get(
        server_url + "/api/render/courses",
        headers={
            "Content-Type": "application/json",
            "Accept": "application/json",
            "Authorization": f"Bearer {components.generate_jwt({'subscription': session.get('subscription')}, secret_key=secret_key)}",
        },
    )
    response = lambda: components.get_component(
        session=session,
...
def students_panel(lang):
    ...
    r = requests.get(
        server_url + f"/api/render/courses?subscription={session.get('subscription')}",
        headers={
            "Content-Type": "application/json",
            "Accept": "application/json",
            "Authorization": f"Bearer {components.generate_jwt({'subscription': session.get('subscription')}, secret_key=secret_key)}",
...
def students_task_default():
    ...
    r = requests.get(
        server_url + f"/api/render/course?course_id={course_id}",
        headers={
            "Content-Type": "application/json",
            "Accept": "application/json",
            "Authorization": f"Bearer {components.generate_jwt({'subscription': session.get('subscription')}, secret_key=secret_key)}",
...
def dashboard_students_class():
    ...
    response = lambda: components.get_component(
        session=session,
        component="horizontal-header.html",
        secret_key=secret_key,
        server_url=server_url,
    )
    result, status = components.retry(response, 3, 200)
...
def dashboard_students_specific_class(class_courses):
    ...
    return render_template(
        "es/dashboard_class_section.html", data=r.json(), header=result
    )
