...
def info():
    query = (
        ...
        .select("link, date, hour")
        .eq("user_id", session.get("id"))
        .eq("status", 1)
    )
...
def add_meet():
    ...
    query = supabase.table("meetings").insert(
        {
...
def confirm_meet():
    ...
    calendar_request = requests.put(
        server_url + "/api/calendar/add/event",
